  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"profproject\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(profproject_M, (NULL));
        profproject_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(profproject_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(profproject_M)
                == (NULL)) || rtmGetStopRequested(profproject_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(profproject_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
